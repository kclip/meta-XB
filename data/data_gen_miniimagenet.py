# taken and adapted from https://github.com/yaoyao-liu/mini-imagenet-tools
import os
import random
import numpy as np
...
class Data_gen_miniimagenet:
    def __init__(self, N, mode, num_classes, supp_and_query, num_total_task, k_fold_ind=None): 
        #print('check num_te so that it can be consistent with others -- so that we can compute m_i in auxiliary!')
        self.num_classes = num_classes # way num
        self.vanilla_dataloader = MiniImageNetDataLoader(shot_num=1, way_num=num_classes, episode_test_sample_num=math.ceil(supp_and_query/num_classes + 1), shuffle_images=False, k_fold_ind=k_fold_ind) # test epi 1 per class # as we are using quite many supp_and_query, this is ok as i.i.d. sampling, of course, we will suffle for N and num_te
        self.vanilla_dataloader.generate_data_list(phase=mode, episode_num=num_total_task) # default: 20000 takss
        self.vanilla_dataloader.load_list(phase=mode)
        self.num_total_tasks = num_total_task
        self.mode = mode
        self.N = N
    ...
    def gen(self, N, device):
        if N is None:
            ...
            pass
        # N: size of training set, num_te: size of testing points
        # mode: 'train', 'val', 'test'
        curr_rand_task_ind = int(torch.randperm(self.num_total_tasks)[0])
        _, _, img, label = self.vanilla_dataloader.get_batch(phase=self.mode, idx=curr_rand_task_ind)
        rand_perm_samples = torch.randperm(self.supp_and_query)
        # now split into N and num_te_for_gen
        train_set_indices = rand_perm_samples[:N]
        test_points_indices = rand_perm_samples[N:]
        tr_dataset = (img[train_set_indices].to(device), label[train_set_indices].to(device))
        te_dataset = (img[test_points_indices].to(device), label[test_points_indices].to(device))
        return tr_dataset, te_dataset

class MiniImageNetDataLoader(object):
    def __init__(self, shot_num, way_num, episode_test_sample_num, shuffle_images = True, k_fold_ind=None):
        ...
        self.k_fold_ind = k_fold_ind
        ##### preliminaries for miniimagenet, please check https://github.com/yaoyao-liu/mini-imagenet-tools
        metatrain_folder = '/DIR_FOR_IMAGNET/mini-imagenet-tools/processed_images/train'
        metaval_folder = '/DIR_FOR_IMAGNET/mini-imagenet-tools/processed_images/val'
        metatest_folder = '/DIR_FOR_IMAGNET/mini-imagenet-tools/processed_images/test'

        npy_dir = '/DIR_FOR_IMAGNET/mini-imagenet-tools/episode_filename_list/new/'
        if not os.path.exists(npy_dir):
            os.mkdir(npy_dir)

        self.npy_base_dir = npy_dir + str(self.shot_num) + 'shot_' + str(self.way_num) + 'way_' + str(episode_test_sample_num) + 'shuffled_' + str(self.shuffle_images) + 'k_fold_' + str(self.k_fold_ind) + '/' # k_fold_ for meta-VB (meta-tr_benchmark)
    ...
    def get_batch(self, phase='train', idx=0):
        ...
        this_inputa = None #torch.from_numpy(this_inputa).permute(0,3,1,2).float()
        this_labela = None #torch.from_numpy(this_labela).float()

        this_inputb = torch.from_numpy(this_inputb).permute(0,3,1,2).float()
        this_labelb = torch.from_numpy(this_labelb).float() # (num_samples, num_classes)
        this_labelb = torch.max(this_labelb, 1)[1]
        this_labelb = this_labelb.unsqueeze(dim=1) # (num_samples, 1)
